# load data ----
# cross-sectional
dat_cs <- read_dta("dat/PSRM_Replication_Data.dta") %>%
  mutate(
    IGUNS13_ordinal = case_when(
      IGUNS13 == 5 ~ 1,
      IGUNS13 == 4 ~ 2,
      IGUNS13 == 3 ~ 3,
      IGUNS13 == 2 ~ 4,
    ...
    IGUNS13_binary = case_when(
      IGUNS13_ordinal %in% c(4, 5) ~ 1,
...
dat_fd <- dat_cs %>%
  mutate(
    ...
    IGUNS14_binary = case_when(
      IGUNS14_ordinal %in% c(4, 5) ~ 1,
      IGUNS14_ordinal %in% c(1:3) ~ 0
    )
  ) %>%
  filter(
    postevent == 0, # only keep Dec respondents who finished before the shooting
    IGUNS14_ordinal %in% c(1:5)
  ) # drop missing values

# group conditions and variable names
group <- c(
  "postevent %in% c(0, 1)", # trivial condition for full sample
  "democrat == 1",
  "republican == 1",
  "indep == 1",
  "liberal == 1",
...
est_atab5 <- function(formula, lincom) {
  lh_robust(formula,
    linear_hypothesis = lincom,
    data = dat_fd, weights = dat_fd$dec2012wt1, se_type = "stata"
  )
}
atab5_1 <- est_atab5(
  (IGUNS14_ordinal - IGUNS13_ordinal) ~ support,
  "support = 0"
)
atab5_2 <- est_atab5(
  (IGUNS14_ordinal - IGUNS13_ordinal) ~ -1 + democrat + republican + indep,
  "democrat = republican"
)
atab5_3 <- est_atab5(
  (IGUNS14_ordinal - IGUNS13_ordinal) ~ -1 + liberal + moderate + conservative + ideodk,
