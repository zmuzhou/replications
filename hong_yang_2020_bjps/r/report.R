...
set_colnames(c("Min", "Median", "Max", "Mean", "SD", "\\(N\\)")) %>%
  ...
  xtable(digits = c(0, rep(2, 5), 0), align = c("l", rep("r", 6)),
         caption = "Summary Statistics of Variables for the Main Results",
         label = "sumstats") %>%
  print.xtable(sanitize.text.function = function(x){x}, math.style.negative = T,
...
texreg(fits_tab1,
       ...
       custom.note = "Year fixed-effects included. County-clustered standard errors in parentheses. Time-varying variables are one-year lagged. %stars.",
       caption = "Natural Resources and Ethnic Violence in Xinjiang, Negative Binomial Regression Estimation Results",
       caption.above = T, center = F, dcolumn = T, booktabs = T, use.packages = F)
...
texreg(fits_tab7,
       ...
       stars = c(0.01, 0.05, 0.1),
       custom.model.names = c("(1)", "(2)", "(3)"),
       custom.note = "Two-way fixed-effects included. County-clustered standard errors in parentheses. Control Variables are one-year lagged. %stars.",
